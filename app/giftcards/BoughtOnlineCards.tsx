...
const BoughtOnlineCards: React.FC = () => {
  const {
    onlineCards,
    onlineCardsLoading: loading,
    onlineCardsError: error,
    fetchOnlineCards
  } = useAppStore();
  ...
  const renderItem = ({ item, index }: { item: any; index: number }) => {
    const logoUrl = item.productDetails?.logoUrls?.[0];
    const orderId = item.customIdentifier || item.id;
      ...
      >
        ...
        <View style={styles.cardContent}>
          ...
          <View style={styles.cardFooter}>
            ...
            <View style={[
              styles.statusBadge, 
              { backgroundColor: item.status === 'SUCCESSFUL' ? '#dcfce7' : (isDark ? '#374151' : '#f3f4f6') }
            ]}>
              <View style={[
                styles.statusIndicator,
                { backgroundColor: item.status === 'SUCCESSFUL' ? '#16a34a' : '#6b7280' }
              ]} />
              <Text style={[
                styles.statusText,
                { color: item.status === 'SUCCESSFUL' ? '#16a34a' : '#6b7280' }
              ]}>
                {item.status === 'SUCCESSFUL' ? 'Active' : item.status}
              </Text>
            </View>
          </View>
        </View>
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={[styles.emptyText, { color: colorScheme === 'dark' ? '#9ca3af' : '#6b7280' }]}>
              No online gift cards found
            </Text>
            <Text style={[styles.emptySubtext, { color: colorScheme === 'dark' ? '#6b7280' : '#9ca3af' }]}>
              Your purchased online gift cards will appear here
            </Text>
          </View>
        }
      />
...
export default BoughtOnlineCards;
